...
elon_musk_chain = CharCreationChain.from_description(
    char_descriptions['elon_musk'])
rick_sanchez_chain = CharCreationChain.from_description(
    char_descriptions['rick_sanchez'])
socrates_chain = CharCreationChain.from_description(
    char_descriptions['socrates'])
marie_curie_chain = CharCreationChain.from_description(
    char_descriptions['marie_curie'])
isaac_newton_chain = CharCreationChain.from_description(
    char_descriptions['isaac_newton'])
taylor_swift_chain = CharCreationChain.from_description(
    char_descriptions['taylor_swift'])
sheldon_cooper_chain = CharCreationChain.from_description(
    char_descriptions['sheldon_cooper'])


def main():
    st.title("Talk to your Idol!🤖")
    st.write(
        "🚀 This app lets you chat with AI-generated versions of famous characters.")
...
#         st.session_state.char = 'sheldon_cooper'
    ...
    if st.session_state.char:
        ...
        query = st.text_input(
            f"You can now chat with {st.session_state.char}", key="input", placeholder="Ask your question!")
    ...
    linkedin_url = "https://www.linkedin.com/in/akshat-anand/"
    st.markdown(f"Reach out to me on [LinkedIn]({linkedin_url})")
    add_vertical_space(2)
    st.markdown(
        "Want personal assistant for your work? Join [Instaminutes](https://www.instaminutes.com/)")
